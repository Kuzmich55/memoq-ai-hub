...
`v1.0.20` 增加上传 TM/TMX 匹配能力，用于 AI prompt 参考：

- Custom TM 资产现在支持 TMX 文件，包括 memoQ 导出的 UTF-16 TMX 和相邻上下文 metadata。
- 桌面端会计算可解释的 `AI Hub TM score`，并使用 memoQ 风格区间：`101%`、`100%`、`95-99`、`85-94`、`75-84`、`<75`。
- 上传 TM 命中会与 memoQ SDK 的 TM hint 分开处理，并写入翻译历史，便于诊断 AI 参考了哪些记忆库内容。
